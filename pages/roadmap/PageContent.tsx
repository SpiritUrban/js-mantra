...
const QuestionPanel = dynamic(() => import("@/components/organisms/QuestionPanel"), { ssr: false });
...
const JavaScriptCourse: React.FC = () => {
  ...
  const currentPoint = () => {
    return (
      javascriptCourse?.[topicPointer]?.subtopics?.[subtopicPointer]?.points?.[pointPointer] ||
      { questions: [] }
    );
  };
